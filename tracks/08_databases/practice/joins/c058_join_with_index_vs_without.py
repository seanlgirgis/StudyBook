# Story:
# This file compares join performance with and without an index.
# It matters because the same join can be fast or slow depending on the index.
# Expect the indexed join to be faster and use fewer rows.
...
def run_join_perf_demo():
    # Step 0:
    # Reset data and stats.
    _setup_tables()

    join_sql = """
        SELECT c.id, o.total_cents
        FROM customers c
    ...
    # Case A:
    # With index: expect lower total time.
    _add_index_on_orders()
    _print_plan(
        "Case A - with index (expect faster join)",
        join_sql,
    )

    # Case B:
    # Without index: expect higher time and Hash Join.
    _drop_index_on_orders()
    _print_plan(
        "Case B - without index (expect slower join)",
        join_sql,
    )
...
if __name__ == "__main__":
    # Step 1:
    # Run the join performance demo directly.
    run_join_perf_demo()

# Takeaway:
# Indexes can dramatically change join cost and strategy.
