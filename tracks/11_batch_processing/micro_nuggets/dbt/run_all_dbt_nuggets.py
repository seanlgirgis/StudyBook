"""
Run all dbt micro-nugget scripts with one-line status output per script.

Usage:
    python run_all_dbt_nuggets.py
    python run_all_dbt_nuggets.py --timeout 180
    python run_all_dbt_nuggets.py --stop-on-fail
    python run_all_dbt_nuggets.py --show-pass-output
...
def discover_nuggets(root: Path, skip_setup: bool = False) -> list[Path]:
    """
    Find all .py nugget scripts under root, sorted deterministically.

    Excludes:
      - _dbt_lane_connect.py (connection module)
      - run_all_dbt_nuggets.py (this runner)
      - 99_reset_lab.py (requires --confirm flag; run manually)
      - __pycache__ directories
      - 00_setup/ directory when skip_setup=True
    """
    scripts = []
    for path in root.rglob("*.py"):
        ...
        if "__pycache__" in parts:
            continue
        # Exclude virtual environments (.venv_dbt, .venv, venv, etc.)
        if any(p.startswith(".venv") or p in {"venv", "env"} for p in parts):
            continue
        # Exclude lab_dbt_project/ (dbt project files, not nugget scripts)
        if parts[0] == "lab_dbt_project":
            continue
        # Optionally skip setup scripts
        if skip_setup and parts[0] == "00_setup":
            continue
        # Exclude reset script (requires --confirm flag)
        if rel.name == "99_reset_lab.py":
            continue

        scripts.append(path)

    # Deterministic order: sort by relative path string (folder prefix sorts numerically)
...
def run_one(script: Path, root: Path, timeout_s: int) -> NuggetResult:
    """Run a single nugget script as a subprocess; capture output."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run all dbt micro-nugget scripts with one-line status output."
    )
    parser.add_argument(
        "--timeout",
        type=int,
    ...
    parser.add_argument(
        "--skip-setup",
        action="store_true",
        help="Skip 00_setup/ scripts (useful if the lab is already seeded).",
    )
    ...
    safe_print(f"dbt nuggets discovered: {len(scripts)}")
    safe_print("-" * 80)
