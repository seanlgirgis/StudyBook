...
| C-003 | P1 | validation | Snowflake read-only connectivity check | cloud account metadata + encrypted secrets | `poc/connection_proofs/python/snowflake_connection_proof.py` | command succeeds or records blocker | done | 2026-04-02 owner-shell proof returned `ok: true`; encrypted SNOWFLAKE_* resolved and query returned account/user/role/warehouse/region (`QU00939`, `SEANLGIRGIS`, `ACCOUNTADMIN`, `COMPUTE_WH`, `AWS_US_EAST_2`) |
...
| M-002 | P0 | migration | Shift/lift coding challenge roadmaps | workspace challenge assets | `D:\StudyBook\coding_challenges` | migration script + inventory parity + source absence check | done | Executed via `scripts/migration/run_m002_coding_assets.ps1 -Execute -DeleteSource`; run=`run_20260402_113935`; move_map_entries=688; conflicts=0; secret_hits=0; backup=`C:\Users\shareuser\migration_backups\m002_backup_20260402_113935` |
| M-002b | P1 | migration | Shift/lift Python fundamentals + Python data libraries into coding_challenges python island | `D:\Workspace\Basics\Language\Python` + `D:\Workspace\Basics\Python_Data` | `D:\StudyBook\coding_challenges\python` | file presence + inventory check | done | Included in M-002 run `run_20260402_113935` |
...
| M-008 | P2 | migration | Shift/lift interview assets | workspace interview sources | `D:\StudyBook\interview` | markdown/notebook completeness checks | done | Executed via `scripts/migration/run_m011_m013_m008_mlai.ps1 -Execute`; run=`run_20260402_121903`; source_de_interview_notebooks=21; moved with deterministic `de_interview_` prefixes ; source deletion complete via move-map report `delete_tech_deinterview_report.json` |
...
| M-011 | P0 | migration | Shift/lift validated Technologies R1/R3 notebooks | `D:\Workspace\Technologies\*.ipynb` | `D:\StudyBook\tracks\10/11/12/22/29/30` + `interview` | notebook smoke runs per track | done | Executed via `scripts/migration/run_m011_m013_m008_mlai.ps1 -Execute`; run=`run_20260402_121903`; source_tech_notebooks=54; move_map=`D:\StudyBook\temp\migration_meta\run_20260402_121903\move_map.csv` ; source deletion complete via move-map report `delete_tech_deinterview_report.json` |
| M-012 | P1 | migration | Shift/lift Databases notebooks + prompt packs | `D:\Workspace\Basics\Databases\*.ipynb` + prompts | `D:\StudyBook\tracks\08_databases` + `_prompts\legacy\databases` | inventory parity + target secret scan + file presence checks | done | Executed via `scripts/migration/run_m012_databases_assets.ps1 -Execute`; run=`run_20260402_120828`; source_ipynb_count=15; source_prompt_count=59; move_map_entries=74; secret_hits=0 |
| M-013 | P1 | migration | Shift/lift Technologies prompts into legacy archive and derive canonical prompts | `D:\Workspace\Technologies\prompts\R1/R2/R3` | `D:\StudyBook\_prompts\legacy\technologies` + `_prompts\tracks` | prompt contract checks | done | Executed via `scripts/migration/run_m011_m013_m008_mlai.ps1 -Execute`; run=`run_20260402_121903`; source_tech_prompts_r123=88; conflicts=88 resolved via deterministic `__dupNNN` suffixing ; source deletion complete via move-map report `delete_tech_deinterview_report.json` |
| M-014 | P0 | security | Block secret-bearing files from migration and sanitize hardcoded creds | `tech.env`, `gcp_key.json`, prompt/notebook literals | migrated StudyBook artifacts | secret scan + manual review | in_progress | Secret scan run for M-011/M-013/M-008/ML_AI migration with `secret_hits=0` in `D:\StudyBook\temp\migration_meta\run_20260402_121903\secret_scan_hits.json`; keep in progress until remaining migration waves complete |
| M-015 | P1 | migration | Shift/lift ML_AI pack into canonical ml platform track | `D:\Workspace\ML_AI` | `D:\StudyBook\tracks\22_ml_platform\ml_ai_pack` | inventory parity + source absence check | done | Executed via `scripts/migration/run_m011_m013_m008_mlai.ps1 -Execute -DeleteMlAiSource`; run=`run_20260402_121903`; source_ml_ai_files=26; `D:\Workspace\ML_AI` deleted after backup at `C:\Users\shareuser\migration_backups\ml_ai_backup_20260402_121903` |
...
| BATCH-MIG-02A | Validated notebook shift/lift | M-011 | infra baseline passing | validated notebooks moved and smoke-tested | done | Completed by `run_m011_m013_m008_mlai.ps1` run=`run_20260402_121903` |
| BATCH-MIG-03A | Prompt shift/lift and canonical derivation | M-012, M-013, M-014 | notebook migration stable | legacy prompts archived + canonical prompts created + secret gate passed | in_progress | M-012 + M-013 complete; migration secret gate passed on this wave (`secret_hits=0`), keep in progress for remaining prompt migrations outside Technologies/Databases |
