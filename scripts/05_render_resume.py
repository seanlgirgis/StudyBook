...
def load_resume_intermediate(job_dir: Path) -> Dict[str, Any]:
    generated_dir = job_dir / "generated"
    if not generated_dir.is_dir():
        raise FileNotFoundError(f"No generated/ folder: {generated_dir}")

    exact = generated_dir / "resume_intermediate.json"
    if exact.is_file():
        print(f"Using: {exact.name}")
        with open(exact, encoding="utf-8") as f:
            return json.load(f)

    files = sorted(generated_dir.glob("resume_intermediate*.json"), reverse=True)
    if not files:
...
def generate_files(tailored: Dict, job_folder: Path, trim_mode: bool, exclusions: Set[str], to_pdf: bool = False):
    md_path = job_folder / "generated" / "resume.md"
    docx_path = job_folder / "generated" / "resume.docx"
...
def main():
    ...
    parser.add_argument("--full", action="store_true", help="Render full experience list (default is trimmed top 5)")
    parser.add_argument("--format", default="docx", choices=["docx", "pdf"])
    args = parser.parse_args()

    job_folder = resolve_job_folder(args.uuid)
    tailored = load_resume_intermediate(job_folder)
    exclusions = load_exclusions()

    to_pdf = args.format == "pdf"

    trim_mode = not args.full
    generate_files(tailored, job_folder, trim_mode, exclusions, to_pdf)
