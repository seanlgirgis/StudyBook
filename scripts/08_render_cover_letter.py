...
def load_cover_intermediate(job_dir: Path) -> dict:
    ...
    exact_path = generated_dir / "cover_intermediate.json"
    ...
    # Fallback to latest
    files = sorted(generated_dir.glob("cover_intermediate*.json"), reverse=True)
    if not files:
...
def main():
    ...
    cover = load_cover_intermediate(job_folder)

    # Markdown preview
    md_path = job_folder / "generated" / "cover.md"
