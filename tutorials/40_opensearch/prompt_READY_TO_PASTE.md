...
D:\Workarea\StudyBook\tutorials\40_opensearch\

TOPIC: OpenSearch for Data Engineers
SLUG: 40_opensearch
PRIORITY: Advanced Cloud NICE
INFRASTRUCTURE: Local JSON generation; OpenSearch endpoint optional
...
===== FILE 01: 01_index_mappings.py =====
PURPOSE: Build OpenSearch index mappings.
COVERS: mappings, keyword vs text, date fields

EXACT FUNCTION SIGNATURES:
    def pipeline_log_mapping() -> dict:
    def transaction_search_mapping() -> dict:
...
===== FILE 02: 02_bulk_indexing.py =====
PURPOSE: Build bulk indexing payloads.
COVERS: bulk API, action metadata, idempotent IDs

EXACT FUNCTION SIGNATURES:
    def build_bulk_action(index_name: str, document_id: str, document: dict) -> str:
    def build_bulk_payload(index_name: str, documents: list[dict], id_field: str) -> str:
    def count_bulk_operations(payload: str) -> int:
...
===== FILE 03: 03_search_queries.py =====
PURPOSE: Create search DSL queries.
COVERS: match, term, range, bool queries

EXACT FUNCTION SIGNATURES:
    def term_query(field: str, value: str) -> dict:
    def range_query(field: str, gte: str | None = None, lte: str | None = None) -> dict:
    def pipeline_error_search(pipeline_name: str, start_time: str, end_time: str) -> dict:
...
===== FILE 04: 04_aggregations_dashboards.py =====
PURPOSE: Build aggregation queries for dashboards.
COVERS: terms aggregation, date histogram, metrics

EXACT FUNCTION SIGNATURES:
    def errors_by_pipeline_aggregation() -> dict:
    def latency_histogram_aggregation(interval: str) -> dict:
    def quality_score_stats_aggregation() -> dict:
...
===== FILE 05: 05_operations_security.py =====
PURPOSE: Explain shards, replicas, ILM, and security.
COVERS: shards, replicas, retention, access control

EXACT FUNCTION SIGNATURES:
    def recommend_shard_count(index_size_gb: float) -> int:
    def build_retention_policy(days: int) -> dict:
    def redact_search_document(document: dict) -> dict:
...
============================================================
CAPSTONE SCENARIO: Index logs and searchable pipeline metadata into OpenSearch.
...
--- capstone/pipeline_search_platform.py ---
EXACT FUNCTION SIGNATURES:
    def build_index_assets(index_name: str) -> dict[str, dict]:
    def build_ingest_payload(logs: list[dict], index_name: str) -> str:
    def build_observability_queries() -> dict[str, dict]:
    def validate_search_assets(assets: dict[str, dict]) -> dict[str, bool]:
REQUIREMENTS:
...
- assert mapping has date timestamp
- assert bulk payload has action and document lines
- assert error search uses bool filter
- assert aggregation includes date_histogram
- assert redaction removes email/account fields
