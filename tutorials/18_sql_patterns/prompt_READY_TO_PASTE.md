...
D:\Workarea\StudyBook\tutorials\18_sql_patterns\

TOPIC: SQL Patterns for Data Engineering Interviews
SLUG: 18_sql_patterns
PRIORITY: Capital One MUST
INFRASTRUCTURE: DuckDB local, no external database required
...
===== FILE 01: 01_window_functions.py =====
PURPOSE: Teach ranking, running totals, lag/lead, and frames.
COVERS: window functions, frames, partition/order

EXACT FUNCTION SIGNATURES:
    def create_sales_data() -> pd.DataFrame:
    def top_n_per_customer_query(n: int) -> str:
    def running_total_query() -> str:
...
===== FILE 02: 02_ctes_and_recursive_queries.py =====
PURPOSE: Use CTEs to organize complex transformations.
COVERS: CTEs, recursive CTE, readability

EXACT FUNCTION SIGNATURES:
    def customer_lifetime_value_query() -> str:
    def recursive_calendar_query(start_date: str, end_date: str) -> str:
    def explain_cte_benefits() -> list[str]:
...
===== FILE 03: 03_joins_and_lateral_patterns.py =====
PURPOSE: Explain join semantics and lateral-like patterns.
COVERS: inner/left/semi/anti joins, exploding arrays

EXACT FUNCTION SIGNATURES:
    def build_join_fixture() -> dict[str, pd.DataFrame]:
    def unmatched_records_query() -> str:
    def latest_record_per_key_query() -> str:
...
===== FILE 04: 04_scd_and_snapshot_patterns.py =====
PURPOSE: Implement slowly changing dimension logic.
COVERS: SCD Type 1/2, effective dating, snapshots

EXACT FUNCTION SIGNATURES:
    def build_scd_type2_query() -> str:
    def identify_changed_records(current_df: pd.DataFrame, incoming_df: pd.DataFrame) -> pd.DataFrame:
    def close_open_records_query() -> str:
...
===== FILE 05: 05_sessionization_and_funnels.py =====
PURPOSE: Implement sessionization and funnel analytics.
COVERS: event gaps, conversion funnels, attribution

EXACT FUNCTION SIGNATURES:
    def sessionization_query(timeout_minutes: int) -> str:
    def funnel_conversion_query() -> str:
    def calculate_conversion_rates(funnel_df: pd.DataFrame) -> dict[str, float]:
...
============================================================
CAPSTONE SCENARIO: Solve interview-grade analytics problems over orders, events, customers, and card transactions.
...
--- capstone/interview_sql_workbook.py ---
EXACT FUNCTION SIGNATURES:
    def create_interview_dataset() -> dict[str, pd.DataFrame]:
    def answer_revenue_by_cohort() -> pd.DataFrame:
    def answer_latest_status_per_account() -> pd.DataFrame:
    def answer_sessionized_checkout_funnel() -> pd.DataFrame:
    def run_all_sql_patterns() -> dict[str, pd.DataFrame]:
REQUIREMENTS:
...
- assert top-N query returns expected rows
- assert running total final value matches total sales
- assert SCD change detection finds expected changed customers
- assert session count matches fixture
- assert funnel conversion rates are between 0 and 1
