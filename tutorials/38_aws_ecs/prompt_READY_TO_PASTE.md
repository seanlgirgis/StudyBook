...
D:\Workarea\StudyBook\tutorials\38_aws_ecs\

TOPIC: AWS ECS for Data Engineers
SLUG: 38_aws_ecs
PRIORITY: Advanced Cloud SHOULD
INFRASTRUCTURE: AWS concepts + Docker; tests validate task definitions locally
...
===== FILE 01: 01_task_definition.py =====
PURPOSE: Build ECS task definition JSON.
COVERS: task definitions, containers, CPU/memory, logs

EXACT FUNCTION SIGNATURES:
    def build_container_definition(name: str, image: str, command: list[str]) -> dict:
    def build_fargate_task_definition(family: str, container: dict, task_role_arn: str, execution_role_arn: str) -> dict:
    def validate_task_definition(task_def: dict) -> bool:
...
===== FILE 02: 02_ecs_service_vs_task.py =====
PURPOSE: Explain services vs one-off tasks.
COVERS: run-task, services, scheduling, data jobs

EXACT FUNCTION SIGNATURES:
    def build_run_task_request(cluster: str, task_definition: str, subnet_ids: list[str]) -> dict:
    def build_service_request(cluster: str, service_name: str, task_definition: str) -> dict:
    def recommend_ecs_pattern(workload_type: str) -> str:
...
===== FILE 03: 03_logging_and_env.py =====
PURPOSE: Configure env vars, secrets, and CloudWatch logs.
COVERS: logConfiguration, secrets, environment

EXACT FUNCTION SIGNATURES:
    def cloudwatch_log_config(log_group: str, region: str, stream_prefix: str) -> dict:
    def environment_variables(env: dict[str, str]) -> list[dict]:
    def secret_references(secrets: dict[str, str]) -> list[dict]:
...
===== FILE 04: 04_networking_security.py =====
PURPOSE: Teach VPC networking and IAM roles.
COVERS: subnets, security groups, public IP, roles

EXACT FUNCTION SIGNATURES:
    def awsvpc_configuration(subnet_ids: list[str], security_group_ids: list[str], assign_public_ip: bool) -> dict:
    def validate_network_config(config: dict) -> bool:
    def explain_task_role_vs_execution_role() -> list[str]:
...
===== FILE 05: 05_cost_and_scaling.py =====
PURPOSE: Explain Fargate cost and scaling tradeoffs.
COVERS: CPU/memory, scheduled tasks, autoscaling

EXACT FUNCTION SIGNATURES:
    def estimate_fargate_task_cost(vcpu: float, memory_gb: float, hours: float) -> float:
    def recommend_task_size(input_gb: float) -> dict[str, float]:
    def build_scheduled_task_rule(schedule_expression: str) -> dict:
...
============================================================
CAPSTONE SCENARIO: Package and run containerized data jobs on ECS/Fargate.
...
--- capstone/ecs_data_job_definition.py ---
EXACT FUNCTION SIGNATURES:
    def build_data_job_task_definition(config: dict) -> dict:
    def build_data_job_run_request(config: dict) -> dict:
    def validate_data_job_security(task_definition: dict) -> dict[str, bool]:
    def write_ecs_artifacts(output_dir: str, config: dict) -> list[str]:
REQUIREMENTS:
...
- assert log config uses awslogs
- assert no plaintext secret values in env
- assert network config includes subnets
- assert cost estimate is positive
