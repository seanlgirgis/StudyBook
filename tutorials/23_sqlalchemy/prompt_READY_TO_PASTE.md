...
D:\Workarea\StudyBook\tutorials\23_sqlalchemy\

TOPIC: SQLAlchemy for Data Engineers
SLUG: 23_sqlalchemy
PRIORITY: Capital One MUST
INFRASTRUCTURE: SQLite by default; Postgres optional; SQLAlchemy 2.x
...
===== FILE 01: 01_engine_metadata_core.py =====
PURPOSE: Create engines, metadata, and Core tables.
COVERS: SQLAlchemy Core, metadata, connections, transactions

EXACT FUNCTION SIGNATURES:
    def create_sqlite_engine(db_path: str):
    def build_metadata() -> MetaData:
    def create_pipeline_runs_table(metadata: MetaData) -> Table:
...
===== FILE 02: 02_core_insert_select.py =====
PURPOSE: Insert and query records using Core.
COVERS: insert, select, transactions, result mappings

EXACT FUNCTION SIGNATURES:
    def insert_pipeline_run(conn, table: Table, run_id: str, status: str) -> int:
    def fetch_pipeline_runs(conn, table: Table) -> list[dict]:
    def update_pipeline_status(conn, table: Table, run_id: str, status: str) -> int:
...
===== FILE 03: 03_orm_models_sessions.py =====
PURPOSE: Teach ORM models and session lifecycle.
COVERS: declarative models, sessions, unit of work

EXACT FUNCTION SIGNATURES:
    def create_session_factory(engine):
    def create_job_record(session, job_name: str, status: str) -> int:
    def list_job_records(session) -> list[dict]:
...
===== FILE 04: 04_bulk_operations.py =====
PURPOSE: Compare row-by-row and bulk insert patterns.
COVERS: bulk insert, executemany, performance considerations

EXACT FUNCTION SIGNATURES:
    def generate_event_rows(row_count: int) -> list[dict]:
    def bulk_insert_events(conn, table: Table, rows: list[dict]) -> int:
    def count_events(conn, table: Table) -> int:
...
===== FILE 05: 05_reflection_and_migrations.py =====
PURPOSE: Reflect existing tables and explain migration workflow.
COVERS: reflection, Alembic concepts, schema drift

EXACT FUNCTION SIGNATURES:
    def reflect_table(engine, table_name: str) -> Table:
    def compare_columns(expected: list[str], reflected: list[str]) -> dict[str, list[str]]:
    def generate_add_column_migration_sql(table_name: str, column_name: str, column_type: str) -> str:
...
============================================================
CAPSTONE SCENARIO: Use SQLAlchemy Core and ORM patterns for reliable pipeline metadata and bulk operations.
...
--- capstone/pipeline_metadata_store.py ---
EXACT FUNCTION SIGNATURES:
    def initialize_metadata_store(db_path: str):
    def record_pipeline_start(engine, pipeline_name: str) -> str:
    def record_pipeline_finish(engine, run_id: str, status: str, metrics: dict) -> None:
    def get_pipeline_history(engine, pipeline_name: str) -> list[dict]:
REQUIREMENTS:
...
- assert metadata tables are created
- assert pipeline start returns run id
- assert finish updates status and metrics
- assert history returns chronological records
