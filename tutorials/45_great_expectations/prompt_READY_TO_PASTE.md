...
D:\Workarea\StudyBook\tutorials\45_great_expectations\

TOPIC: Great Expectations for Data Quality
SLUG: 45_great_expectations
PRIORITY: Advanced Cloud SHOULD
INFRASTRUCTURE: Pure Python: pandas; Great Expectations concepts; tests can validate expectation dictionaries
...
===== FILE 01: 01_expectation_basics.py =====
PURPOSE: Create basic expectation definitions.
COVERS: expectations, suites, validation results

EXACT FUNCTION SIGNATURES:
    def expect_column_to_exist(column: str) -> dict:
    def expect_values_not_null(column: str) -> dict:
    def expect_values_between(column: str, min_value: float, max_value: float) -> dict:
...
===== FILE 02: 02_pandas_quality_checks.py =====
PURPOSE: Run quality checks on pandas dataframes.
COVERS: schema checks, null checks, range checks

EXACT FUNCTION SIGNATURES:
    def validate_required_columns(df: pd.DataFrame, required_columns: list[str]) -> dict[str, bool]:
    def validate_not_null(df: pd.DataFrame, columns: list[str]) -> dict[str, int]:
    def validate_numeric_ranges(df: pd.DataFrame, ranges: dict[str, tuple[float, float]]) -> dict[str, int]:
...
===== FILE 03: 03_expectation_suites.py =====
PURPOSE: Build reusable expectation suites.
COVERS: suite design, naming, ownership

EXACT FUNCTION SIGNATURES:
    def build_transactions_suite() -> list[dict]:
    def build_customers_suite() -> list[dict]:
    def summarize_suite(suite: list[dict]) -> dict[str, int]:
...
===== FILE 04: 04_checkpoint_reporting.py =====
PURPOSE: Create checkpoint-style validation reports.
COVERS: checkpoints, pass/fail, failure summaries

EXACT FUNCTION SIGNATURES:
    def run_validation_checkpoint(df: pd.DataFrame, suite: list[dict]) -> dict:
    def build_failure_report(validation_result: dict) -> pd.DataFrame:
    def validation_pass_rate(validation_result: dict) -> float:
...
===== FILE 05: 05_data_docs_and_alerts.py =====
PURPOSE: Generate data-doc-style summaries and alert payloads.
COVERS: data docs, Slack/email payloads, severity

EXACT FUNCTION SIGNATURES:
    def build_data_docs_summary(validation_result: dict) -> str:
    def classify_quality_severity(pass_rate: float) -> str:
    def build_alert_payload(validation_result: dict) -> dict:
...
============================================================
CAPSTONE SCENARIO: Build a data quality suite for batch pipeline outputs with actionable failure reporting.
...
--- capstone/quality_gate.py ---
EXACT FUNCTION SIGNATURES:
    def build_quality_gate_suite() -> list[dict]:
    def run_quality_gate(df: pd.DataFrame) -> dict:
    def should_block_pipeline(validation_result: dict, threshold: float = 0.95) -> bool:
    def write_quality_report(validation_result: dict, output_path: str) -> str:
REQUIREMENTS:
...
- assert expectation dictionaries have expectation_type
- assert required column validation catches missing column
- assert range validation catches negative amount
- assert pass rate is exact for fixture
- assert pipeline blocks below threshold
