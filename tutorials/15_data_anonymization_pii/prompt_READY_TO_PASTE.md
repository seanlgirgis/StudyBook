...
D:\Workarea\StudyBook\tutorials\15_data_anonymization_pii\

TOPIC: Data Anonymization & PII Handling
SLUG: 15_data_anonymization_pii
PRIORITY: Capital One MUST
INFRASTRUCTURE: Pure Python: pandas, faker, hashlib, re; optional presidio mentioned conceptually only
...
===== FILE 01: 01_generate_pii_dataset.py =====
PURPOSE: Generate realistic customer/transaction data containing direct and quasi-identifiers.
COVERS: faker schemas, deterministic seeds, PII vs quasi-identifier labeling

EXACT FUNCTION SIGNATURES:
    def generate_customer_dataset(row_count: int, seed: int = 42) -> pd.DataFrame:
    def add_quasi_identifiers(df: pd.DataFrame) -> pd.DataFrame:
...
===== FILE 02: 02_detect_pii.py =====
PURPOSE: Detect likely PII columns using regex and column-name heuristics.
COVERS: PII classification, regex patterns, false positives, explainability

EXACT FUNCTION SIGNATURES:
    def classify_column_name(column_name: str) -> str:
    def sample_values(df: pd.DataFrame, column: str, sample_size: int = 20) -> list[str]:
...
===== FILE 03: 03_masking_hashing.py =====
PURPOSE: Apply masking and hashing strategies to PII fields.
COVERS: irreversible hashing, masking, salted hashes, deterministic pseudonyms

EXACT FUNCTION SIGNATURES:
    def mask_email(email: str) -> str:
    def mask_phone(phone: str) -> str:
    def hash_value(value: str, salt: str) -> str:
    def anonymize_direct_identifiers(df: pd.DataFrame, salt: str) -> pd.DataFrame:
...
===== FILE 04: 04_tokenization_vault.py =====
PURPOSE: Demonstrate tokenization with a local in-memory token vault.
COVERS: token vaults, reversibility boundaries, operational risks

EXACT FUNCTION SIGNATURES:
    def create_token(value: str, prefix: str) -> str:
    def tokenize_column(df: pd.DataFrame, column: str, prefix: str) -> tuple[pd.DataFrame, dict[str, str]]:
    def detokenize_value(token: str, vault: dict[str, str]) -> str | None:
...
===== FILE 05: 05_k_anonymity_risk.py =====
PURPOSE: Evaluate quasi-identifier groups and enforce k-anonymity.
COVERS: k-anonymity, suppression, generalization, re-identification risk

EXACT FUNCTION SIGNATURES:
    def calculate_equivalence_classes(df: pd.DataFrame, quasi_columns: list[str]) -> pd.DataFrame:
    def enforce_k_anonymity(df: pd.DataFrame, quasi_columns: list[str], k: int) -> pd.DataFrame:
    def privacy_risk_report(original_df: pd.DataFrame, anonymized_df: pd.DataFrame, quasi_columns: list[str]) -> dict[str, float]:
...
============================================================
CAPSTONE SCENARIO: Build a privacy-safe customer analytics pipeline for card transaction and customer service data.
...
--- capstone/pii_pipeline.py ---
EXACT FUNCTION SIGNATURES:
    def load_raw_customer_data(path: str) -> pd.DataFrame:
    def anonymize_customer_data(df: pd.DataFrame, salt: str, k: int = 3) -> pd.DataFrame:
    def validate_no_plaintext_pii(df: pd.DataFrame) -> bool:
    def write_safe_dataset(df: pd.DataFrame, output_path: str) -> str:
    def run_pipeline(input_path: str, output_path: str, salt: str) -> pd.DataFrame:
REQUIREMENTS:
...
- assert no output email contains '@'
- assert SSN column is absent or hashed
- assert k-anonymity minimum equivalence class is >= 3
- assert row count is not zero
- assert safe dataset has expected columns
