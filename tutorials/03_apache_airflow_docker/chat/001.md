Here is **File 01 adapted for your Airflow Docker environment** (paths + behavior aligned with containers, not local standalone).

---

## 📄 `studybook_01_dag_basics.py`
...
* DAG appears (if not → import issue)
* Tasks run in order:

  ```
  extract → transform → load → check_output_files → notify
  ```
* Logs show file paths like:

  ```
  /tmp/studybook/01_dag_basics/...
  ```
...
* “DAG not visible”
* or paste error from UI

I’ll debug it with you immediately.
...
Next step = **Operators + Sensors (real Airflow power)**
