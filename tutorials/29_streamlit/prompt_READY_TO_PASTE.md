...
D:\Workarea\StudyBook\tutorials\29_streamlit\

TOPIC: Streamlit for Data Apps
SLUG: 29_streamlit
PRIORITY: DE Fundamentals NICE
INFRASTRUCTURE: Pure Python: streamlit, pandas, plotly optional; tests focus on pure functions
...
===== FILE 01: 01_app_structure_state.py =====
PURPOSE: Create Streamlit app structure with testable helper functions.
COVERS: app layout, session state, separation of UI and logic

EXACT FUNCTION SIGNATURES:
    def load_sample_metrics() -> pd.DataFrame:
    def filter_metrics(df: pd.DataFrame, pipeline_name: str | None) -> pd.DataFrame:
...
===== FILE 02: 02_data_loading_caching.py =====
PURPOSE: Teach caching and safe data loading.
COVERS: st.cache_data concepts, CSV loading, validation

EXACT FUNCTION SIGNATURES:
    def read_metrics_csv(path: str) -> pd.DataFrame:
    def validate_metrics_frame(df: pd.DataFrame) -> bool:
    def prepare_cached_dataset(path: str) -> pd.DataFrame:
...
===== FILE 03: 03_quality_visualizations.py =====
PURPOSE: Build chart-ready datasets for quality visualization.
COVERS: trend data, chart inputs, KPI cards

EXACT FUNCTION SIGNATURES:
    def build_kpi_cards(df: pd.DataFrame) -> dict[str, str]:
    def build_quality_trend(df: pd.DataFrame) -> pd.DataFrame:
    def build_failure_breakdown(df: pd.DataFrame) -> pd.DataFrame:
...
===== FILE 04: 04_interactive_filters.py =====
PURPOSE: Implement filter logic independent of UI widgets.
COVERS: date ranges, multiselect filters, search
...
===== FILE 05: 05_deployment_config.py =====
PURPOSE: Explain deployment and config patterns.
COVERS: secrets, requirements, health checks, production limits

EXACT FUNCTION SIGNATURES:
    def build_requirements() -> list[str]:
    def build_streamlit_config() -> str:
...
============================================================
CAPSTONE SCENARIO: Build a lightweight data quality dashboard for pipeline outputs.
...
--- capstone/data_quality_app.py ---
EXACT FUNCTION SIGNATURES:
    def create_demo_quality_dataset(row_count: int = 100) -> pd.DataFrame:
    def calculate_dashboard_metrics(df: pd.DataFrame) -> dict[str, object]:
...
- assert dashboard metrics include pass rate
- assert filters reduce rows correctly
- assert alert table only contains failing pipelines
- assert config contains headless true
- assert secret redaction hides middle characters
