...
D:\Workarea\StudyBook\tutorials\28_data_stubbing\

TOPIC: Data Stubbing and Test Data Generation
SLUG: 28_data_stubbing
PRIORITY: DE Fundamentals SHOULD
INFRASTRUCTURE: Pure Python: faker, pandas, json
...
===== FILE 01: 01_stub_schemas.py =====
PURPOSE: Define reusable schemas for fake data.
COVERS: schema design, field constraints, deterministic seeds

EXACT FUNCTION SIGNATURES:
    def customer_schema() -> dict[str, str]:
    def transaction_schema() -> dict[str, str]:
...
===== FILE 02: 02_faker_generators.py =====
PURPOSE: Generate realistic rows with Faker.
COVERS: faker providers, seeds, repeatability

EXACT FUNCTION SIGNATURES:
    def generate_customer_rows(row_count: int, seed: int = 42) -> list[dict]:
    def generate_transaction_rows(customers: list[dict], row_count: int, seed: int = 42) -> list[dict]:
    def write_jsonl(rows: list[dict], path: str) -> str:
...
===== FILE 03: 03_edge_case_injection.py =====
PURPOSE: Inject edge cases for testing pipelines.
COVERS: nulls, duplicates, invalid values, late events

EXACT FUNCTION SIGNATURES:
    def inject_nulls(rows: list[dict], field: str, count: int) -> list[dict]:
    def inject_duplicates(rows: list[dict], count: int) -> list[dict]:
    def inject_invalid_amounts(rows: list[dict], count: int) -> list[dict]:
...
===== FILE 04: 04_contract_fixtures.py =====
PURPOSE: Build contract fixtures for producer/consumer tests.
COVERS: contracts, required fields, schema drift

EXACT FUNCTION SIGNATURES:
    def required_fields_contract() -> dict[str, list[str]]:
    def validate_rows_against_contract(rows: list[dict], contract: dict[str, list[str]]) -> dict[str, int]:
    def generate_contract_breaking_rows() -> list[dict]:
...
===== FILE 05: 05_synthetic_data_quality.py =====
PURPOSE: Measure quality of generated test data.
COVERS: coverage, distributions, uniqueness

EXACT FUNCTION SIGNATURES:
    def uniqueness_report(rows: list[dict], key_field: str) -> dict[str, float]:
    def categorical_distribution(rows: list[dict], field: str) -> dict[str, int]:
    def synthetic_quality_score(rows: list[dict]) -> float:
...
============================================================
CAPSTONE SCENARIO: Generate realistic stub data for pipelines before real systems are available.
...
--- capstone/stub_data_factory.py ---
EXACT FUNCTION SIGNATURES:
    def build_full_stub_dataset(customer_count: int, transaction_count: int, output_dir: str) -> dict[str, str]:
    def inject_pipeline_test_cases(transaction_path: str) -> str:
    def validate_stub_dataset(output_dir: str) -> dict[str, bool]:
    def generate_readme_summary(output_dir: str) -> str:
REQUIREMENTS:
...
- assert customer and transaction files exist
- assert generated data is deterministic with same seed
- assert injected duplicates increase duplicate count
- assert contract validator catches missing fields
- assert quality score is above threshold
