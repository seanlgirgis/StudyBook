...
D:\Workarea\StudyBook\tutorials\20_pydantic\

TOPIC: Pydantic for Data Validation
SLUG: 20_pydantic
PRIORITY: Capital One SHOULD
INFRASTRUCTURE: Pure Python: pydantic v2, pandas optional
...
===== FILE 01: 01_models_and_field_validation.py =====
PURPOSE: Define strict models for event records.
COVERS: BaseModel, Field, validators, type coercion

EXACT FUNCTION SIGNATURES:
    def build_transaction_payload() -> dict:
    def validate_transaction(payload: dict) -> TransactionEvent:
    def summarize_validation_error(error: Exception) -> list[str]:
...
===== FILE 02: 02_nested_models_and_enums.py =====
PURPOSE: Model nested payloads and controlled vocabularies.
COVERS: nested models, enums, optional fields

EXACT FUNCTION SIGNATURES:
    def validate_customer_profile(payload: dict) -> CustomerProfile:
    def flatten_customer_profile(profile: CustomerProfile) -> dict:
    def allowed_risk_levels() -> list[str]:
...
===== FILE 03: 03_batch_validation.py =====
PURPOSE: Validate batches and separate good/bad records.
COVERS: batch validation, dead-letter records, error reporting

EXACT FUNCTION SIGNATURES:
    def validate_batch(payloads: list[dict]) -> tuple[list[dict], list[dict]]:
    def error_report(dead_letters: list[dict]) -> pd.DataFrame:
    def calculate_quality_score(valid_count: int, invalid_count: int) -> float:
...
===== FILE 04: 04_settings_management.py =====
PURPOSE: Use Pydantic settings-style patterns for config.
COVERS: env config, defaults, secrets hygiene

EXACT FUNCTION SIGNATURES:
    def load_pipeline_config(env: dict[str, str]) -> PipelineConfig:
    def redact_config(config: PipelineConfig) -> dict:
    def validate_required_settings(config: PipelineConfig) -> bool:
...
===== FILE 05: 05_schema_generation_contracts.py =====
PURPOSE: Generate JSON schema contracts from models.
COVERS: JSON schema, producer/consumer contracts

EXACT FUNCTION SIGNATURES:
    def transaction_json_schema() -> dict:
    def check_required_fields(schema: dict, expected_fields: list[str]) -> bool:
    def compare_schema_versions(old_schema: dict, new_schema: dict) -> dict[str, list[str]]:
...
============================================================
CAPSTONE SCENARIO: Validate incoming event payloads before loading them into a data platform.
...
--- capstone/validated_ingestion.py ---
EXACT FUNCTION SIGNATURES:
    def validate_transaction_events(events: list[dict]) -> tuple[list[dict], list[dict]]:
    def write_dead_letter_report(dead_letters: list[dict], output_path: str) -> str:
    def publish_contract_schema(output_path: str) -> str:
    def run_validated_ingestion(events: list[dict], output_dir: str) -> dict[str, object]:
REQUIREMENTS:
...
- assert valid transaction passes
- assert invalid currency fails
- assert batch splits valid and invalid records
- assert quality score is exact for fixture
- assert generated schema contains required fields
