"""03e_char_tfidf_typo_search: build typo-tolerant character TF-IDF index and demo candidate matches."""
...
def get_default_paths(project_root: Path) -> tuple[Path, Path, Path, Path]:
    """Return default input/index/metadata/sample-result paths for this POC."""

    input_path = project_root / "pocs" / "03c_text_normalization" / "outputs" / "normalized_chunks.json"
    index_output = project_root / "pocs" / "03e_char_tfidf_typo_search" / "outputs" / "char_tfidf_index.joblib"
    metadata_output = project_root / "pocs" / "03e_char_tfidf_typo_search" / "outputs" / "char_index_metadata.json"
    sample_output = project_root / "pocs" / "03e_char_tfidf_typo_search" / "outputs" / "sample_typo_search_results.json"
    return input_path, index_output, metadata_output, sample_output
...
def build_char_tfidf_index(
    ...
    """Build character-level TF-IDF artifact from validated chunk records."""

    if not chunks:
        raise ValueError("Cannot build character TF-IDF index from empty chunk list")
...
def normalize_query_for_search(query: str) -> str:
    """Apply light query normalization compatible with 03c lowercase text."""
...
def search_char_tfidf(artifact: dict[str, Any], query: str, top_k: int = 3) -> dict[str, Any]:
    """Return top-k typo-tolerant candidate chunk matches for one query."""
    ...
    for rank_index, (row_index, score) in enumerate(ranked, start=1):
        chunk_meta = metadata[row_index]
        top_matches.append(
            {
                "rank": rank_index,
...
def create_sample_typo_search_results(
    artifact: dict[str, Any],
    queries: list[str] | None = None,
    top_k: int = 3,
) -> list[dict[str, Any]]:
    """Generate sample typo-search candidate matches for learning/debugging."""

    search_queries = queries if queries is not None else list(DEFAULT_SAMPLE_QUERIES)
    return [search_char_tfidf(artifact=artifact, query=query, top_k=top_k) for query in search_queries]
...
def create_index_metadata(
    ...
    """Create human-readable metadata for the saved character TF-IDF artifact."""
    ...
    return {
        "poc": "03e_char_tfidf_typo_search",
        "input_path": _display_path(input_path, project_root),
        "index_path": _display_path(index_path, project_root),
        "sample_results_path": _display_path(sample_results_path, project_root),
...
def save_char_tfidf_artifact(artifact: dict[str, Any], output_path: Path) -> None:
    """Persist character TF-IDF artifact to joblib file."""
...
def save_index_metadata(metadata: dict[str, Any], output_path: Path) -> None:
    """Persist character index metadata as pretty JSON."""

    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(json.dumps(metadata, indent=2), encoding="utf-8")


def save_sample_results(results: list[dict[str, Any]], output_path: Path) -> None:
    """Persist sample typo-search candidate results as pretty JSON."""
...
def parse_args(project_root: Path) -> argparse.Namespace:
    """Parse CLI arguments with repo-relative defaults and optional overrides."""

    default_input, default_index, default_metadata, default_sample = get_default_paths(project_root)

    parser = argparse.ArgumentParser(description="Build typo-tolerant character TF-IDF index from normalized chunks.")
    parser.add_argument(
        "--input",
    ...
    parser.add_argument(
        ...
        default=default_sample.relative_to(project_root).as_posix(),
        help="Path to sample typo-search JSON (absolute or repo-relative)",
    )
    return parser.parse_args()


def main() -> None:
    """Thin CLI wrapper for the 03e character TF-IDF build pipeline."""
    ...
    sample_results_output_path = resolve_cli_path(project_root, args.sample_results_output).resolve()

    chunks = load_normalized_chunks(input_path)
    artifact, vectorizer_config = build_char_tfidf_index(chunks)

    save_char_tfidf_artifact(artifact, index_output_path)
    sample_results = create_sample_typo_search_results(artifact)
    save_sample_results(sample_results, sample_results_output_path)
    metadata = create_index_metadata(
        artifact=artifact,
        input_path=input_path,
        index_path=index_output_path,
        sample_results_path=sample_results_output_path,
    ...
    print(f"Output sample results path: {sample_results_output_path}")
    print(f"Chunks read: {len(chunks)}")
    print(f"Character TF-IDF matrix shape: {artifact['matrix'].shape}")
    print(f"Vocabulary size: {len(artifact['vectorizer'].vocabulary_)}")
    print(f"Sample typo queries run: {DEFAULT_SAMPLE_QUERIES}")
    print("PASS")
