...
def canFinish(numCourses: int, prerequisites: List[List[int]]) -> bool:
    ...
    #
    # Question becomes:
    # "If I start from any course and keep walking prerequisite links,
    #  do I ever come back to a node already in my CURRENT walk?"
    #
    # If yes, that's a cycle -> impossible to finish all courses.
    # If no cycles anywhere, schedule is valid.
    graph_map = {i: [] for i in range(numCourses)}
    
    # Build adjacency list: course -> list of direct prerequisites.
    # Example [3,1] means: to take 3, you must take 1 first.
    for course, pre_req in prerequisites:
        graph_map[course].append(pre_req)
    
    # 0 = unvisited
    # 1 = done
    # 2 = on current DFS path (visiting). Seeing 2 again means a cycle.
    
    visit_set = [0] * numCourses
    
    def has_cycle(course: int) -> bool:
        # Enter node: mark as "currently exploring this path".
        visit_set[course] = 2
        
        for pre in graph_map[course]:
            # If prerequisite is already "visiting", we looped back in same path.
            # Visual: A -> ... -> pre, and now pre points back into active chain.
            if visit_set[pre] == 2:
                return True
            # If prerequisite not processed yet, DFS into it.
            if visit_set[pre] == 0 and has_cycle(pre):
                return True
        
        # All prerequisite paths from this node are clean.
        # Mark "done" so future DFS calls can skip it.
        visit_set[course] = 1
        return False

    for course in range(numCourses):
        # Try DFS from every disconnected component.
        # Skip nodes already marked done from earlier traversals.
        if visit_set[course] == 0 and has_cycle(course):
            return False
