# LeetCode 49: Group Anagrams (Review Drill)
    ...
    for i, (words, expected_groups, expected_sizes) in enumerate(test_cases):
        try:
            ...
            if len(result) == expected_groups and sizes == expected_sizes:
                ...
                print(f"    Normalized output:    {_normalize(result)}")
...
def groupAnagrams(strs: List[str]) -> List[List[str]]:
    ...
    Typical pattern:
    - Build a hash map where key = an anagram signature
    - Append each word to the matching bucket
    """
    def make_me_tuple(s)-> Tuple[int, ...]:
    ...
    for s in strs:
        tpl = make_me_tuple(s)
        counts.setdefault(tpl, []).append(s)  
