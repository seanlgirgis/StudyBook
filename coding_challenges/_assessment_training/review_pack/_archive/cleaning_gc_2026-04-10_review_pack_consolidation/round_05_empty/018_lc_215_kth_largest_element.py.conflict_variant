...
def findKthLargest(nums: List[int], k: int) -> int:
    # Keep a min-heap of size k to track the k largest numbers seen so far.
    # If heap size exceeds k, pop the smallest value.
    # At all times, the heap contains only the k biggest values.
    # The heap top is the smallest among them (the k-th largest overall).
    lst = []
    for n in nums:
