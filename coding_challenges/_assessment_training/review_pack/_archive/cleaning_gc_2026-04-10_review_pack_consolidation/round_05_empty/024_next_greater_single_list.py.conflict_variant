...
def nextGreaterRight(nums: List[int]) -> List[int]:
    res = [-1] * len(nums)
    stack = []  # stores INDICES, values are decreasing bottom→top

    for i, num in enumerate(nums):
        # While current value > value at stack top → I am their answer
        while stack and nums[stack[-1]] < num:
            idx = stack.pop()
            res[idx] = num
        stack.append(i)

    # Anything left in stack → no greater element found → stays -1
    return res
