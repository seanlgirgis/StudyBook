...
def largestRectangleArea(heights: List[int]) -> int:
    # Solution on two steps
    # A use mono increasing stack in way up; Inherit last index 
    # B Clean up stack 
    max_rect = 0
    stack = []          # mono increasing stack of tuple () height, index if cur h is less than top .. pop and processs
    l = len(heights)
    ...
    for i , h in enumerate(heights):
        ...
        while stack and stack[-1][0] > h:
            ...
            inh_ind = pop_i
            max_rect = max(max_rect, pop_h * (i- pop_i))
