...
capstone_integration:
  - "Monotonic stack (LC739 sliding-window-max pattern) for real-time CPU anomaly detection"
  - "Anti-join SQL query to find servers with missing daily_metrics reports"
  - "@timer and @retry decorators wrapping each pipeline stage"
  - "Lambda architecture pattern — batch layer (SQL aggregates) + speed layer (streaming alerts)"
