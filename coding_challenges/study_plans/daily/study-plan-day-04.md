...
```yaml
day: 04
output_dir: D:\Workspace\DaysStudyGemini2\Day-04
theme: "Binary Search, SQL Query Optimization, NumPy & Vectorization, APM & Observability"
leetcode:
  - id: LC704  slug: binary_search
  - id: LC074  slug: search_a_2d_matrix
  - id: LC153  slug: find_minimum_in_rotated_sorted_array
  - id: LC033  slug: search_in_rotated_sorted_array
  - id: LC981  slug: time_based_key_value_store
sql_slug: query_optimization
python_slug: numpy_vectorization
tech_slug: apm_observability
capstone_name: telemetry_capacity_analyzer
capstone_integration:
  - "Binary search (LC981 floor-query pattern) → sorted threshold config lookup in TelemetryAnalyzer.get_threshold(tier, metric) at O(log n)"
  - "SQL covering index + EXPLAIN ANALYZE → optimize daily_metrics aggregation query from 45s full scan to 0.3s index seek"
  - "NumPy vectorization → vectorized CPU stats (mean, std, 95th percentile) across the full 6000-server capacity matrix"
  - "APM trend-based alerting → capstone AlertEngine uses rolling 7-day baseline (window function) to trigger deviation alerts instead of static thresholds"
design_pattern: "Pipeline Pattern with Strategy Pattern for validation and computation"
shared_domain:
...
> **BONUS VARIANT for Gemini:** Generate a bonus solution for `Search in Rotated Sorted Array (LC33)` as a natural extension of LC704.
> The input is a sorted array that has been rotated at an unknown pivot. Key insight: one half is always sorted; check which half, then determine which half contains the target.
> Add Q: "How does binary search extend to rotated arrays?" after the main solution.
...
> **BONUS VARIANT for Gemini:** Generate a bonus solution for `Search a 2D Matrix II (LC240)` — the staircase search variant.
> The matrix rows are sorted but the inter-row property does NOT hold (row[i][0] is NOT > row[i-1][-1]).
> Key insight: start at top-right corner; if target < current move left, if target > current move down. O(m+n).
> Contrast with LC74: show why 1D binary search fails here but staircase succeeds.
...
> **BONUS VARIANT for Gemini:** Generate a bonus solution for `Find Minimum in Rotated Sorted Array II (LC154)` — same problem but the array may contain duplicates.
> Key insight: when `nums[mid] == nums[right]`, we cannot determine which half is sorted — must shrink the window with `right -= 1`.
> Show that duplicates degrade worst-case to O(n) (e.g., [2,2,2,0,2]) but average case remains O(log n).
...
> **BONUS VARIANT for Gemini:** Generate a bonus solution for `Search in Rotated Sorted Array with Duplicates (LC81)`.
> Duplicates mean `nums[left] == nums[mid]` is ambiguous — can't determine which half is sorted.
> Key fix: when `nums[left] == nums[mid]`, increment left (`left += 1`) and continue.
> Show that this reduces to O(n) worst case for all-identical arrays but handles the general case correctly.
...
> **BONUS VARIANT for Gemini:** Generate a bonus solution for `Snapshot Array (LC1146)`.
> Design a data structure that supports `set(index, val)`, `snap()` (returns snap_id), and `get(index, snap_id)`.
> Key insight: store a list of (snap_id, val) per index — only on change. `get()` uses binary search on snap_ids for the floor query (largest snap_id ≤ requested snap_id).
> This is the direct generalization of LC981's floor-query pattern applied to versioned data.
...
-- Optimization target: SELECT server_id, AVG(avg_cpu) FROM daily_metrics WHERE report_date = ? GROUP BY server_id
```
...
1. **"Binary search works on any monotonic condition — not just sorted arrays. Define your search space, write the condition, eliminate half."**
2. **"Rotated sorted arrays always have one fully-sorted half. Identify which half with one comparison, range-check the target, eliminate the other."**
3. **"LC981 floor query = bisect_right minus 1. Every monitoring system does this: 'give me the state at or before time T.' O(log n)."**
4. **"EXPLAIN ANALYZE before touching indexes. Know the difference between Sequential Scan (bad for large tables), Index Scan, and Index Only Scan (covering index — best)."**
5. **"NumPy vectorization is C under the hood. For 6M values, a Python loop takes 180s; NumPy takes 1.2s. Never loop over arrays in production data pipelines."**
6. **"APM observability = metrics + logs + traces. Static thresholds cause alert fatigue. Trend-based alerting (deviation from rolling baseline) cuts false positives 70%."**
...
| Topic | Capstone Role |
|---|---|
| **LC981 Binary Search (floor query)** | `TelemetryAnalyzer.get_threshold(tier, metric)` — bisect_right on sorted config table; O(log n) tier-based threshold lookup |
| **SQL Query Optimization** | `analytics.py` uses EXPLAIN ANALYZE to verify the `CREATE INDEX idx_dm_server_date ON daily_metrics(server_id, report_date)` covering index is used for all aggregation queries |
| **NumPy Vectorization** | `capacity_matrix.py` computes `np.mean`, `np.std`, `np.percentile(matrix, 95, axis=0)` across the 6000×1440 server-by-minute matrix in < 2s |
| **APM Trend-Based Alerting** | `AlertEngine.detect_trend_deviation()` uses a 7-day rolling baseline (SQL window function) and triggers only when current avg_cpu deviates > 2 std from the baseline — no static thresholds |

> **Capstone name:** `telemetry_capacity_analyzer` — see `capstone/mini_project_brief.md`
> **Design pattern:** Pipeline Pattern with Strategy Pattern for validation and computation
