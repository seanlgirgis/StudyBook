...
```yaml
day: "05"
output_dir: D:\Workspace\DaysStudyGemini2\Day-05
theme: "Heap/Priority Queue, Analytical SQL (YoY/Cohort/Funnel), ETL Patterns, Capacity Planning"
leetcode:
  - id: LC703   slug: kth_largest_element_in_a_stream
  - id: LC1046  slug: last_stone_weight
  - id: LC973   slug: k_closest_points_to_origin
  - id: LC215   slug: kth_largest_element_in_an_array
  - id: LC295   slug: find_median_from_data_stream
sql_slug: analytical_sql
python_slug: etl_patterns
tech_slug: capacity_planning
capstone_name: telemetry_capacity_planner
capstone_integration:
  - "Heap top-K algorithm → find_top_k_servers() tracks the K most overloaded servers by avg_cpu using a min-heap of size K in O(n log k) per batch"
  - "YoY/cohort SQL patterns → query_yoy_growth() compares each server's current-month avg_cpu to the same month last year; cohort query groups servers by provisioning quarter"
  - "ETL pipeline with argparse/logging → run_pipeline() CLI orchestrator accepts --top-k, --threshold, --dry-run flags with structured logger.info() at every stage boundary"
  - "Capacity Planning 4-step loop (Baseline→Model→Forecast→Result) → shapes the 4-stage pipeline architecture of the entire capstone"
design_pattern: "Pipeline Pattern with Strategy Pattern for validation"
shared_domain:
...
**Answer:** A heap of size 1 always holds the single maximum element seen so far — `add()` becomes a running maximum tracker, returning the largest value in the entire stream at every call. The underlying principle is that when k=1, the kth largest IS the maximum, so the min-heap's root (its only element) always equals the global maximum by definition. In production, this pattern models a real-time peak CPU tracker — Citi's alerting pipeline used this exact structure to maintain the highest observed CPU reading per server without storing the full history, triggering a P1 alert whenever the running maximum crossed 95%.
...
> **BONUS VARIANT for Gemini:** Generate a bonus solution for a **Running P95 Tracker**.
> Extend `KthLargest` to track the **95th percentile** of a stream. P95 of n elements = the (⌈0.05 × n⌉)th smallest = the (⌊0.95 × n⌋ + 1)th largest. The heap size is dynamic — as n grows, k = max(1, int(0.05 * n)) grows too. Add a `get_p95()` method and demonstrate with Citi's 6,000 CPU readings scenario where the P95 threshold is used as a dynamic alerting cutoff instead of a fixed static value.
...
**Answer:** `heapify` rearranges a list in-place into a valid min-heap in O(n) time — not O(n log n), which surprises engineers who assume building a heap requires n individual insertions. The O(n) proof works because the bottom-up sift-down approach does proportionally less work at lower levels: the majority of nodes near the leaves only need 1–2 swaps, while only O(1) nodes near the root face full O(log n) work — the geometric series sum converges to O(n). In production ETL pipelines, calling `heapq.heapify(batch)` at startup costs O(n) to initialize from a historical dataset, far cheaper than n individual `heappush` calls when bootstrapping Citi's capacity tracker from 90 days of APM data at pipeline start.

---

### Q3: What is the time complexity of the optimal approach and why?

**Answer:** O(n log n) overall. `heapify` builds the initial heap in O(n); each of at most n-1 turns does 2 pops + at most 1 push, each O(log n) on a shrinking heap. Total: O(n) + O(n × log n) = O(n log n) — far better than the brute force O(n² log n) of repeatedly scanning for the two largest. The insight is that the heap shrinks by at least 1 element each round, so log(heap size) decreases over time, but the dominant cost is still the first round's O(log n). In Citi's alert deduplication system, this stone-smashing pattern modeled event priority merging — reducing 10,000 overlapping threshold alerts into a single highest-priority Jira ticket in O(n log n) time.
...
> **BONUS VARIANT for Gemini:** Generate a bonus solution for **Last Stone Weight II (LC1049)**.
> Split the stones into two groups to minimize the absolute difference of their sums. Note: this is **not** a heap problem — it's a DP subset-sum problem. Recognizing when a "stone/weight" framing requires DP instead of a greedy heap is a key senior-level skill. Target sum = total_sum // 2. Build a DP boolean table and find the largest achievable sum ≤ target.
...
> **BONUS VARIANT for Gemini:** Generate a bonus solution for **K Closest Points using QuickSelect** instead of a heap.
> Implement the partition-based O(n) average-time solution. Pick a random pivot point, partition by squared distance, then recurse only on the side that contains k. Expected O(n) average vs O(n log k) for the heap — but O(n²) worst case without randomization. Contrast the tradeoffs: when does each approach win in a production streaming context?
...
> **BONUS VARIANT for Gemini:** Generate a bonus solution for the **K-Way Merge problem**.
> Given K sorted arrays, merge them into one fully sorted array using a min-heap. Each heap entry should be `(value, array_index, element_index)` so you can efficiently pull the next element from the contributing array. Time: O(N log K) where N = total elements across all arrays. This models merging K daily telemetry batch files (each sorted by report_date) into one chronological stream.
...
**Answer:** `addNum` = O(log n) — at most 2–3 heap operations (heappush + heappop for rebalancing), each O(log n) on heaps of size ≤ n. `findMedian` = O(1) — just reads the tops of both heaps without modifying them, making it safe to call at any query frequency. Space = O(n) — unavoidable, since the median always requires knowledge of the element distribution; unlike count or sum, it can't be computed from a constant-size summary. In Citi's rolling SLO monitoring system, this O(1) `findMedian` cost meant the P50 CPU response dashboard could refresh every second across 6,000 servers without adding measurable latency to the query path, even as `addNum` continued ingesting 6,000 new readings per minute.
...
> **BONUS VARIANT for Gemini:** Generate a bonus solution for **Sliding Window Median (LC480)**.
> Extend `MedianFinder` to track the median over only the **last K elements** in a stream (not all elements). The challenge: heaps don't support efficient element removal. Hint: use a lazy deletion map — mark removed elements as "pending delete" and skip them when they surface as the heap root. This simulates O(log K) removal without restructuring the heap.
...
| Topic | Capstone Role |
|-------|--------------|
| Heap top-K (LC703 / LC215) | `find_top_k_servers()` — tracks the K most overloaded servers by `avg_cpu` using a min-heap of size K; O(n log k) per batch, no full sort needed |
| YoY / Cohort SQL | `query_yoy_growth()` — compares each server's current-month `avg_cpu` to the same month last year; cohort query groups by provisioning quarter |
| ETL with argparse / logging | `run_pipeline()` — CLI-driven orchestrator accepting `--top-k`, `--threshold`, `--dry-run` flags; structured `logger.info()` at every stage boundary |
| Capacity Planning 4-step loop | Shapes the entire pipeline: **Baseline** (90-day avg_cpu history) → **Model** (trend slope per server) → **Forecast** (days to 70% ceiling) → **Alert** (Jira ticket if breach < 60 days) |

> **Capstone name:** `telemetry_capacity_planner` — see `capstone/mini_project_brief.md`
> **Design pattern:** Pipeline Pattern with Strategy Pattern for validation
