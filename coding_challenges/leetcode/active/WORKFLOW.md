...
```
<Project_Root>\outbox\study-plan-day-NN.md   (raw)
         │
         ▼  STEP 1: CLAUDE (Pre-Gemini)
         │  Prompt: CLAUDE_PRE_GEMINI_PROMPT.md
         │  Takes ~5 min, uses ~5% of 5hr allocation
         │
<Project_Root>\outbox\study-plan-day-NN.md   (enhanced — same file, overwritten)
         │
         ▼  STEP 2: GEMINI
         │  Prompt: ENHANCED_MASTERPROMPT.md  (paste to Gemini)
         │  Input:  enhanced study-plan-day-NN.md
         │  Output: <Project_Root>\DaysStudy\Day-NN\  (~35 files)
         │  Quality: ~90-94%
         │
<Project_Root>\DaysStudy\Day-NN\      (Gemini output, ~90% quality)
         │
         ...
         │
<Project_Root>\DaysStudy\Day-NN\      (closed, ~99% quality)
```
...
```
prompt > "<Project_Root>\DaysStudy\CLAUDE_PRE_GEMINI_PROMPT.md"
inputFile > "<Project_Root>\outbox\study-plan-day-NN.md"
```
...
`<Project_Root>\DaysStudy\ENHANCED_MASTERPROMPT.md`

At the very end, replace the placeholder with:
```
<Project_Root>\outbox\study-plan-day-NN.md
```
...
Start a new Claude Code session. Paste:
```
prompt > "<Project_Root>\DaysStudy\CLAUDE_POST_GEMINI_PROMPT.md"
inputDir > "<Project_Root>\DaysStudy\Day-NN"
```
...
| `STUDY-PLAN-METADATA-TEMPLATE.md` | Reference for adding metadata to study plans manually. |
| `Day-02/` | Gemini Day-02 output (audited, 94/100). |
| `Day-03/` | Next generation target. |
